.. ifconfig:: persona == 'oem'
   ...
   Every user of |prod| is associated with an account and has two specific parameters which define the 
   ...
   * :guilabel:`Role`: The :term:`Role` defines the action privileges of the user. There are two roles in
      ...
      * :guilabel:`User`: A user with the "User" role has only operational rights on the system data.
      * :guilabel:`Super User`: A user with the "Super User" role has the same rights as users with the
        "User" role, and can additionally manage |prod| users.
